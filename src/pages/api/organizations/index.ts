...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.collection?.length > 0) {
      const create_collection = body.collection;
      body.collection = {
    ...
    if (body?.mp3_player?.length > 0) {
      const create_mp3_player = body.mp3_player;
      body.mp3_player = {
        create: create_mp3_player,
      };
    } else {
      delete body.mp3_player;
    }
