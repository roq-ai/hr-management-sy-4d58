...
import { SongListPage as ShowCaseList } from 'pages/songs';

function DashboardPage() {
  const { session } = useSession();

  const collectionStat = useSWR<PaginatedInterface<CollectionInterface>>(
    () => `/collections`,
    () => getCollections({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Collections',
      result: collectionStat?.data?.totalCount,
      isLoading: collectionStat.isLoading,
